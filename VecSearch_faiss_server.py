...
# 版本号
from outputNumber import SimilarModel
...
'''
class VecSearch:
    def __init__(self, dim=768, nlist=100, metric='INNER_PRODUCT', gpu=-1): #normalize=True,
        self.dim = dim                          # 向量维度
...
def HttpServer (args):
    ...
    def query ():
        ...
        q = bert_client.get_sentence_vec(txt)
        ...
        res["value"]=str(value)  # 0:value
        res["txt"]=str(T)  # 1:txt
        res["result"]='OK' # 2:result
        logging.info('查询结果:%s' % res)
        logging.info('查询用时:%.2f 毫秒' % ((time.time() - start)*1000) )
        # return jsonify(res)

        # list1 = tuple(tuple(x) for x in res.tolist())
